...
export default function SettingsPage() {
  ...
  const ConfigImageSection = ({ title, configKey, currentUrl, icon: Icon }: any) => (
    <div className="admin-glass-card p-6">
      ...
      <div className="flex flex-col md:flex-row gap-6 items-start">
        <div className="w-full md:w-40 aspect-[4/5] rounded-2xl overflow-hidden bg-white/80 backdrop-blur-md border-2 border-dashed border-white flex items-center justify-center relative group">
          {currentUrl ? (
            <img src={currentUrl} alt={title} className="w-full h-full object-cover" />
          ) : (
            <Icon className="h-12 w-12 text-slate-900/20" />
          )}
        ...
        <div className="flex-1 space-y-4">
          <div className="space-y-2">
            <Label className="text-slate-700">Image Asset</Label>
            <div className="flex gap-3">
              <Input 
                ...
                id={`upload-${configKey}`}
              />
              <Button asChild variant="outline" className="border-white text-slate-900 hover:bg-white/80 hover:text-slate-900 bg-white/80 backdrop-blur-md">
            ...
            <p className="text-[11px] text-slate-500"> Recommended aspect ratio 4:5 for portraits, 16:9 for banners.</p>
          </div>
          {currentUrl && (
            <div className="space-y-2">
              <Label className="text-slate-700">Current URL</Label>
              <Input readOnly value={currentUrl} className="rounded-xl border border-white shadow-[inset_0_1px_0_rgba(255,255,255,0.5)] bg-white/80 backdrop-blur-md text-slate-500 text-[10px] h-8 focus-visible:ring-0" />
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <div className="space-y-6 max-w-2xl px-4 md:px-0 text-slate-900 pb-20">
      <div>
        <h2 className="text-3xl font-extrabold text-slate-900">Settings</h2>
        <p className="text-slate-500 text-sm mt-1">Manage institutional and admin preferences</p>
      </div>
      ...
      <div className="pt-8 pb-4">
        ...
        <p className="text-slate-500 text-xs mt-1">Manage images for the homepage activity tiles</p>
      </div>
      ...
      <div className="admin-glass-panel p-6">
        <h3 className="text-base font-bold text-slate-900 mb-5">Website Settings</h3>
        <div className="space-y-5">
          <div className="space-y-2">
            <Label className="text-slate-700">Site Title</Label>
            <Input className="rounded-xl border border-white shadow-[inset_0_1px_0_rgba(255,255,255,0.5)] bg-white/80 backdrop-blur-md focus-visible:bg-white/80 focus-visible:ring-2 focus-visible:ring-slate-300 text-slate-900" defaultValue="SEMCOM - College of Commerce & Management" />
          </div>
          <div className="space-y-2">
            <Label className="text-slate-700">Meta Description</Label>
            <Input className="rounded-xl border border-white shadow-[inset_0_1px_0_rgba(255,255,255,0.5)] bg-white/80 backdrop-blur-md focus-visible:bg-white/80 focus-visible:ring-2 focus-visible:ring-slate-300 text-slate-900" defaultValue="SEMCOM, a college of CVM University, offering BBA, BCA, BCom, MBA, MCom and Ph.D. programs." />
          </div>
          ...
          {[
            ...
            <div key={item.title} className="flex items-center justify-between p-3 rounded-xl bg-white/80 backdrop-blur-md border border-white shadow-[inset_0_1px_0_rgba(255,255,255,0.5)]">
              <div>
                <p className="text-sm font-semibold text-slate-900">{item.title}</p>
                <p className="text-[11px] text-slate-500">{item.desc}</p>
              </div>
              <Switch defaultChecked={item.checked} className="data-[state=checked]:bg-accent data-[state=unchecked]:bg-slate-100" />
            </div>
          ))}
