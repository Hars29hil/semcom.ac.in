...
import { ArrowLeft, Search, Bell, FileText, Loader2 } from 'lucide-react';
...
export default function AnnouncementsList() {
  ...
  useEffect(() => {
    const fetchAnnouncements = async () => {
      try {
        const data = await newsApi.getAnnouncements();
        if (data.success) {
  ...
  const filteredNotes = announcements.filter(note => 
        ...
        <div className="section-container text-center">
          ...
          >
            ...
            <p className="text-muted text-lg max-w-2xl mx-auto">
              Explore our complete collection of announcements, circulars, and updates.
            </p>
          </motion.div>
        </div>
      </section>
          ...
          <div className="bg-surface rounded-2xl shadow-card border border-border p-8 md:p-12 min-h-[600px]">
            ...
            {loading ? (
              <div className="flex flex-col items-center justify-center py-20 gap-4 text-muted">
                ...
                <span className="font-bold text-sm">Loading Announcements...</span>
              </div>
            ) : (
              <div className="space-y-4">
                {filteredNotes.length > 0 ? (
                  filteredNotes.map((note, idx) => {
                    const noteYear = note.date ? new Date(note.date).getFullYear() : new Date().getFullYear();
                    ...
                    <motion.div 
                      ...
                      className="group flex flex-col sm:flex-row gap-6 items-start sm:items-center p-6 bg-background rounded-2xl border border-border hover:border-secondary/30 hover:shadow-sm transition-all"
                    >
                       <div className="flex-shrink-0 w-16 h-16 bg-primary/10 rounded-xl flex flex-col items-center justify-center text-primary group-hover:bg-primary group-hover:text-white transition-colors">
                        {note.type === 'file' ? <FileText size={24} /> : <Bell size={24} />}
                      </div>
                      
                      <div className="flex-grow space-y-2">
                        <div className="flex items-center gap-3">
                          <span className="bg-secondary/10 text-secondary px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider">Announcement</span>
                          <span className="text-xs font-medium text-muted">{noteDate}</span>
                        </div>
                        ...
                        {note.url && (
                          <a href={note.url} target="_blank" rel="noopener noreferrer" className="inline-block mt-2 text-primary font-medium text-sm hover:underline">
                            View Document
                          </a>
                        )}
                      </div>
